...
template <typename Mesh>
void split_longer(Mesh &mesh, float target_length) {
...
template <typename Mesh>
void collapse_shorter(Mesh &mesh, float target_length) {
...
void smooth(Mesh &mesh, float lambda, bool tangential) {
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 10; i > 0; --i) {
        puts("SL");
        split_longer(mesh, 0.001);
        puts("CS");
        collapse_shorter(mesh, 0.001);
        puts("AV");
        adjust_valence(mesh);
        puts("GC");
